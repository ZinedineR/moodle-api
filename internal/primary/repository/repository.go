...
func (r repo) GetQuizUser(ctx context.Context, quizId int, userId int) (*domain.GetQuizUserData, errs.Error) {
	var (
		models       *domain.GetQuizUserData
		QuestionData []domain.QuestionUserData
		quiz         *gorm.DB
		question     *gorm.DB
	)
	query := r.db.WithContext(ctx).
		...
		Group("mdl_quiz.timeopen, mdl_quiz.timeclose, mdl_user.id, mdl_user.firstname, mdl_quiz.course, mdl_course.fullname, mdl_quiz.id, mdl_quiz.name").
		Order("mdl_quiz.id ASC")
	...
	queryquestion := r.db.WithContext(ctx).
		...
		Where("mdl_quiz.id = ?", quizId).
		Where("mdl_user.id = ?", userId).
		Group("mdl_user.id, mdl_quiz.id, mdl_question_attempts.questionsummary, mdl_question.questiontext, mdl_question_attempts.rightanswer").
		Order("mdl_user.id ASC")

	quiz = query.
		Select("mdl_quiz.timeopen as time_open", "mdl_quiz.timeclose as time_close",
			"mdl_user.id as user_id", "concat(mdl_user.firstname,' ',mdl_user.lastname) as name",
			"mdl_quiz.course as course_id", "mdl_course.fullname as course_name",
			"mdl_quiz.id as quiz_id", "mdl_quiz.name as quiz_name").
		Limit(1).
		Find(&models)
...
func (r repo) RedisGetQuizUser(ctx context.Context, quizId int, userId int) (*domain.GetQuizUserData, errs.Error) {
	var (
		models       *domain.GetQuizUserData
		QuestionData []domain.QuestionUserData
		quiz         *gorm.DB
